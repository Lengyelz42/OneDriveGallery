=== UTD OneDrive Gallery Plugin ===
Contributors: Lengyel Zoltán
Tags: onedrive, gallery
Requires at least: 6.0
Tested up to: 6.9
Stable tag: 2.8.3
License: GPLv2 or later
License URI: https://www.gnu.org/licenses/gpl-2.0.html

== Description ==
Displays images and videos from a configured personal OneDrive folder via Microsoft Graph API.

== Installation ==
1. Upload the plugin files to the `/wp-content/plugins/utd-onedrive-gallery` directory, or install the plugin through the WordPress plugins screen directly.
2. Activate the plugin through the 'Plugins' screen in WordPress.

== Changelog ==
= 2.8.2 =
* Stable release.
= 2.8.3 =
* Documentation changes on teh Settings Page.